...
import { Box, ArrowLeft, Activity, Server, Search, LayoutGrid, Cpu, Database } from 'lucide-react';
...
function getMQEValue(data: any, label: string): number {
  try {
    ...
    if (!results || results.length === 0) {
      console.warn(`🔍 DEBUG [${label}]: No results found.`);
      return 0;
    }
    const values = results[0]?.values;
    if (!values || values.length === 0) {
      console.warn(`🔍 DEBUG [${label}]: Empty values array.`);
      return 0;
    }
    const val = values.slice(-1)[0]?.value;
    console.log(`🔍 DEBUG [${label}]: Raw Value ->`, val);
    return val ? parseFloat(val) : 0;
  } catch (e) { 
    console.error(`🔍 DEBUG [${label}]: Parsing Error`, e);
    return 0; 
  }
}
...
export default function K8sNodeExplorerPage() {
  ...
  const [nodeMetrics, setNodeMetrics] = useState({ cpu: 0, mem: 0, totalCores: "0", usedCores: "0" });
  ...
  // Dynamic Cluster Name (No Hardcoding)
  const clusterName = useMemo(() => {
     const k8sSvc = clusterServices.find((s: any) => s.layers?.includes('K8S')) || services.find((s: any) => s.layers?.includes('K8S'));
     const name = k8sSvc?.name || "k8s-cluster::k8s-cluster"; // Fallback just in case
     return name;
  }, [clusterServices, services]);

  // 1️⃣ DYNAMIC WORKLOAD FETCHING
  useEffect(() => {
    async function fetchAllWorkloads() {
      if (services.length === 0) return;
      setLoadingAll(true);
      try {
        const clusterService = clusterServices.find((s: any) => s.layers?.includes('K8S')) 
                            || services.find((s: any) => s.layers?.includes('K8S'));
                            
        let defaultNodeName = "N/A";
        if (clusterService) {
          const nodeRes = await client.query({ 
            query: GET_SERVICE_INSTANCES, 
            variables: { serviceId: clusterService.id, duration: durationObj } 
          });
        ...
        const podResults = await Promise.all(
          podServices.map((svc: any) =>
            client.query({ query: GET_SERVICE_INSTANCES, variables: { serviceId: svc.id, duration: durationObj } })
          )
        );
        ...
        const mergedPods = podResults.flatMap((res: any, i: number) => {
          ...
          return instances.map((inst: any) => {
            ...
            const extractedNode = attrs.find((a: any) => a.name === 'node_name' || a.name === 'host_name')?.value;
            const extractedNs = attrs.find((a: any) => a.name === 'namespace')?.value;
            return {
              ...inst,
              serviceName: podServices[i].name,
              nodeName: extractedNode || defaultNodeName,
              namespace: extractedNs || getNamespace(podServices[i].name),
            };
          });
        });
  ...
  // 3️⃣ 🔥 DYNAMIC METRICS FETCHING (OFFICIAL DOC NAMES)
  useEffect(() => {
    async function fetchNodeMetrics() {
      if (selectedNode === "All" || selectedNode === "N/A") return;
      
      console.group(`🔍 DEBUG: Fetching Metrics for Node: ${selectedNode}`);
      ...
      try {
        const nodeEntity = { 
          ...
          serviceInstanceName: selectedNode, 
          normal: true 
        };

        // Fetching based on official docs provided
        // Usage = k8s_node_cpu_usage (m)
        // Total = k8s_node_cpu_cores (m)
        const [cpuUsageRes, cpuTotalRes, memUsageRes, memTotalRes] = await Promise.all([
          client.query({ query: GET_MQE_METRICS, variables: { expression: "k8s_node_cpu_cores_requests", entity: nodeEntity, duration: durationObj } }),
          client.query({ query: GET_MQE_METRICS, variables: { expression: "k8s_node_cpu_cores", entity: nodeEntity, duration: durationObj } }),
          client.query({ query: GET_MQE_METRICS, variables: { expression: "k8s_node_memory_requests", entity: nodeEntity, duration: durationObj } }),
          client.query({ query: GET_MQE_METRICS, variables: { expression: "k8s_node_memory_total", entity: nodeEntity, duration: durationObj } })
        ]);

        const cUsage = getMQEValue(cpuUsageRes.data, "CPU Usage (m)");
        const cTotal = getMQEValue(cpuTotalRes.data, "CPU Total (m)") || 1; 
        
        const mUsage = getMQEValue(memUsageRes.data, "Mem Usage (Gi)");
        const mTotal = getMQEValue(memTotalRes.data, "Mem Total (Gi)") || 1;

        const cpuPercent = Math.min(Math.round((cUsage / cTotal) * 100), 100);
        const memPercent = Math.min(Math.round((mUsage / mTotal) * 100), 100);

        console.log(`🔍 DEBUG: Calculated -> CPU: ${cpuPercent}% (${cUsage}/${cTotal}), Mem: ${memPercent}% (${mUsage}/${mTotal})`);

        setNodeMetrics({
          cpu: cpuPercent,
          mem: memPercent,
          usedCores: `${Math.round(cUsage)}m`,
          totalCores: `${Math.round(cTotal)}m`
        });

      } catch (e) { 
        console.error("❌ Node Metric fetch FAILED:", e); 
      } finally { 
    ...
    fetchNodeMetrics();
  }, [selectedNode, durationObj, client, clusterName]);

  // 4️⃣ FILTERED DATA
  const filteredPods = useMemo(() => {
    return allPods.filter(pod => {
      const matchNode = selectedNode === "All" || pod.nodeName === selectedNode;
      const matchNs = selectedNamespace === "All" || pod.namespace === selectedNamespace;
      const matchSearch = pod.name.toLowerCase().includes(searchQuery.toLowerCase());
      return matchNode && matchNs && matchSearch;
    });
  }, [allPods, selectedNode, selectedNamespace, searchQuery]);
  ...
  const tableData = useMemo(() => {
    ...
    const breakdown = nodeSpecificPods.reduce((acc: any, pod: any) => {
      if (!acc[pod.namespace]) {
        acc[pod.namespace] = { name: pod.namespace, pods: 0, svcs: new Set(), deps: 0, sts: 0, ds: 0 };
      }
      acc[pod.namespace].pods += 1;
      acc[pod.namespace].svcs.add(pod.serviceName);
      ...
      <div className="max-w-7xl mx-auto space-y-6 px-4 py-8 text-slate-200">
        ...
        <header className="flex justify-between items-center pb-2">
          <div className="space-y-1">
            <button onClick={() => setLocation('/kubernetes')}
              className="text-[10px] font-black text-muted-foreground flex items-center gap-1 uppercase hover:text-blue-400 transition-colors">
              <ArrowLeft size={10} /> CLUSTER
            </button>
            <h1 className="text-3xl font-black text-white italic uppercase tracking-tighter">
              Workload <span className="text-blue-500">Explorer</span>
            </h1>
          </div>
        </header>
        ...
        <Card className="p-4 border-white/5 bg-slate-900/40 backdrop-blur-md">
          <div className="flex flex-wrap items-center gap-4">
            <div className="flex items-center gap-2 text-blue-400">
              <Server size={18} />
              <span className="text-xs font-black uppercase tracking-widest text-white">Target Node:</span>
            </div>
            <div className="flex-1 max-w-[400px]">
              <Select value={selectedNode} onValueChange={setSelectedNode}>
                <SelectTrigger className="bg-black/40 border-white/10 text-white font-bold uppercase text-[11px]">
        ...
        {/* 2. NODE METRICS CARD */}
        {selectedNode !== "All" && (
          <Card className="p-6 bg-blue-500/5 border-blue-500/20 flex flex-col md:flex-row gap-8 items-center relative overflow-hidden group">
            <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
                <Activity size={80} className="text-blue-400" />
            </div>
            ...
            <div className="w-full md:w-2/3 grid grid-cols-1 sm:grid-cols-2 gap-8 z-10">
               <MetricProgress label="Node CPU Allocation" value={nodeMetrics.cpu} loading={loadingMetrics} color="bg-blue-500" />
               <MetricProgress label="Node Memory Allocation" value={nodeMetrics.mem} loading={loadingMetrics} color="bg-cyan-500" />
            </div>
          </Card>
        )}
        ...
        {selectedNode !== "All" && (
          <Card className="border-white/5 bg-slate-900/40 overflow-hidden">
            <div className="p-3 border-b border-white/5 bg-white/5 flex items-center justify-between">
              ...
              <span className="text-[9px] font-bold text-slate-500 uppercase">Per Namespace Analysis</span>
            </div>
                  ...
                  {tableData.map((ns: any) => (
                    <tr key={ns.name} className="hover:bg-white/[0.02] transition-colors group">
                      ...
                      <td className="px-6 py-4 text-center">
                        <span className="bg-blue-500/10 text-blue-400 px-2 py-0.5 rounded-md border border-blue-500/20 font-black text-[10px]">{ns.pods}</span>
                      </td>
        ...
        <div className="space-y-4 pt-4 border-t border-white/5">
          <div className="flex flex-wrap items-center gap-4">
            <div className="flex-1 min-w-[300px] relative group">
              ...
              <Input
                ...
                className="pl-10 h-11 bg-slate-900/60 border-white/10 text-white font-black uppercase text-xs tracking-widest focus-visible:ring-blue-500/50"
              />
            </div>
            <Select value={selectedNamespace} onValueChange={setSelectedNamespace}>
              <SelectTrigger className="w-[200px] h-11 bg-slate-900/60 border-white/10 text-white font-black uppercase text-[10px]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredPods.map((pod: any) => (
              <Card key={pod.id} onClick={() => setLocation(`/kubernetes/namespace/${pod.namespace}/pod/${encodeURIComponent(pod.id)}`)}
                className="p-5 bg-slate-900/40 border-white/5 hover:border-blue-500/30 transition-all cursor-pointer group relative overflow-hidden">
                <div className="flex items-start gap-4 relative z-10">
                  ...
                  <div className="flex-1 min-w-0">
                    <h4 className="font-black text-white italic tracking-tighter truncate uppercase">{pod.name}</h4>
                    <div className="flex items-center gap-2 mt-1">
                        <span className="text-[8px] font-black text-blue-500 bg-blue-500/10 px-1.5 py-0.5 rounded border border-blue-500/10 uppercase tracking-widest">
                            {pod.namespace}
                        </span>
                    </div>
                  </div>
                ...
                <div className="mt-4 pt-4 border-t border-white/5 flex justify-between items-center text-[9px] font-black text-slate-500 uppercase tracking-widest">
                  <span className="truncate max-w-[180px]">{pod.nodeName}</span>
                  <span className="flex items-center gap-1 text-emerald-500 italic"><Activity size={10} /> Active</span>
                </div>
              </Card>
            ))}
          </div>
